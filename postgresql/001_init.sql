...
-- CAMBIO: UNIQUE(ID_DEVICE, COLLECTED_AT_UTC). Habilita ON CONFLICT DO
-- NOTHING en data-reception para que reintentos del buffer local del
-- dispositivo (agregado en un round anterior) no dupliquen filas ni
-- infecten el ROW_COUNT de los agregados horarios.
CREATE TABLE RAM_USAGE (
    ...
    COLLECTED_AT_UTC TIMESTAMP,
    INSERTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (ID_DEVICE, COLLECTED_AT_UTC)
);
...
-- CAMBIO: UNIQUE(ID_DEVICE, DISK_NAME, COLLECTED_AT_UTC).
CREATE TABLE DISK_USAGE (
    ...
    COLLECTED_AT_UTC TIMESTAMP,
    INSERTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (ID_DEVICE, DISK_NAME, COLLECTED_AT_UTC)
);
...
-- CAMBIO: UNIQUE(ID_DEVICE, INTERFACE_NAME, COLLECTED_AT_UTC).
CREATE TABLE NETWORK_INFORMATION (
    ...
    COLLECTED_AT_UTC TIMESTAMP,
    INSERTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (ID_DEVICE, INTERFACE_NAME, COLLECTED_AT_UTC)
);
-- CAMBIO: UNIQUE(ID_DEVICE, SENSOR_KEY, COLLECTED_AT_UTC).
CREATE TABLE CPU_TEMPERATURE (
    ...
    COLLECTED_AT_UTC TIMESTAMP,
    INSERTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (ID_DEVICE, SENSOR_KEY, COLLECTED_AT_UTC)
);
...
-- CAMBIO: UNIQUE(ID_DEVICE, COLLECTED_AT_UTC).
CREATE TABLE UPTIME (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    UPTIME_MINUTES INT,
...
CREATE INDEX idx_threshold_alerts_device_inserted ON THRESHOLD_ALERTS (ID_DEVICE, CREATED_AT_UTC);
CREATE INDEX idx_threshold_alerts_cooldown_lookup
    ON THRESHOLD_ALERTS (ID_DEVICE, ID_PARAMETER, (DATA::jsonb ->> 'Key'), CREATED_AT_UTC)
...
VALUES ('admin', 'admin@ceiot.lse', '$2b$10$apMib6tAp91D42jWTtx1ROBLF1xTvgS1Za8LaH0MOkMAODAdYOWeK', 'admin', 'admin', (SELECT ID FROM ROLES WHERE ROLE_NAME='admin'));